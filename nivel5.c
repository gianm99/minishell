/* nivel5.c - Parte 5 de Aventura 2 - Gian Lucas Martín Chamorro, 
      ...
      if (jobnum)
      {
        ...
        if (WIFEXITED(status))
        {
          fprintf(stderr, "\n[reaper()→ Proceso hijo %d en background (%s) "
                          "finalizado con exit code %d]\n",
                  pid, jobs_list[jobnum].command_line, WEXITSTATUS(status));
          fflush(stderr);
        }
        else if (WIFSIGNALED(status))
        {
          fprintf(stderr, "\n[reaper()→ Proceso hijo %d en background (%s) "
                          "finalizado por señal %d]\n",
                  pid, jobs_list[jobnum].command_line, WTERMSIG(status));
          fflush(stderr);
        }
  ...
  if (jobs_list[0].pid > 0)
  {
    if (strcmp(jobs_list[0].command_line, nombreshell))
    {
      ...
      fprintf(stderr, "[ctrlz()→ Señal %d enviada a %d(%s) por %d(%s)]\n",
              signum, jobs_list[0].pid, jobs_list[0].command_line,
              getpid(), nombreshell);
